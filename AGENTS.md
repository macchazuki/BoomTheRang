...
- Keep JavaScript ES modules, Phaser 4, Vite, Vitest, and HTML/CSS.
- Do not add React/Vue/Svelte, a state library, another tween library, or a physics/collision dependency without a demonstrated need.
...
- Phaser entity/view classes never own XP, progression, hit detection, or authoritative results.
- UI classes never calculate upgrade/reward rules.
- All gameplay timers use elapsed seconds, never frame counts.
- Never grant offline/catch-up dog XP.
- Player hit detection is resolved before visual animation; do not use visual collisions for rewards.
...
- Dispose Phaser game objects/tweens and browser listeners owned by a class.
